...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        Graph g = new Graph(numVertices, false, true);
        MapStrToInt Map = new MapStrToInt();
        for(int i=0; i<numVertices; i++)
        {
           String Neurons = StdIn.readString();
           Map.Add(Neurons, i);
        }
        int numEdges = StdIn.readInt();
        for(int i=0; i<numEdges; i++)
        { 
          String vertice1 = StdIn.readString();
          int v1 = Map.get(vertice1);
          String vertice2 = StdIn.readString();
          int v2 = Map.get(vertice2);
          g.AddEdge(v1, v2);  
        }
    	// WRITE YOUR CODE HERE
      StdOut.setFile(MindStoneNeighborNeuronsOutputFile);
      ArrayList<Integer>DirectConn = g.findDirectMindStConnects();
      for(int i = 0; i<DirectConn.size(); i++)
      {
         StdOut.println(Map.get(DirectConn.get(i)));
      }
